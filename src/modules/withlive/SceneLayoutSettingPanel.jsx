 ...
 * @returns {JSX.Element}
 */
export default function SceneLayoutSettingPanel({
  viewLayout,
  liveList,
  onClose = () => {},
  onSubmit = () => {},
  ...
  useEffect(() => {
    ...
    const waktaverseInfoWithBroadcast = data.members
      .sort(
        (a, b) =>
          Waktaverse.findIndex((member) => member.login_name === a.twitchLoginName) -
          (data.lives.find((item) => item.loginName === a.twitchLoginName)?.broadcaster ? 100 : 0) -
          (Waktaverse.findIndex((member) => member.login_name === b.twitchLoginName) -
            (data.lives.find((item) => item.loginName === b.twitchLoginName)?.broadcaster
              ? 100
              : 0)),
      )
      .map((member) => ({
        ...member,
        broadcaster: data.lives.find((item) => item.loginName === member.twitchLoginName)
          ?.broadcaster,
      }));
  ...
  const memberList = allMemberList
    .filter(
      (member) =>
        !layoutSetting.liveList.includes(member.twitchLoginName) &&
        !(member.twitchLoginName === dragItemState.cursor && dragItemState.toIndex != -1),
    )
    .map((member) => (
      <MemberItem
  ...
  const genePreviewList = (list, dragItemState) => {
    const { mode, cursor, toIndex } = dragItemState;
  ...
  const previewItems = previewList.map((loginName, i) => {
    ...
    return (
      <TransformAnimation
        ...
        onMouseEnter={(e) =>
          dragItemState.mode != 'NONE' &&
          setDragItemState({
            ...dragItemState,
            toIndex: Math.min(layoutSetting.liveList.length, i),
          })
        }
      >
        <div
          ...
          onMouseDown={(e) => {
            !e.target?.className.includes('btnRemove') &&
              selectLiveItem('MOVE', member.twitchLoginName, i);
          }}
        >
          <TransparentButton
            className={styles.btnRemove}
            onClick={(e) => removeLiveItem(member.twitchLoginName)}
          >
            <ClearRoundedIcon className={styles.btnRemoveIcon} />
          </TransparentButton>
          {member && (
            <MemberItem
              loginName={member.twitchLoginName}
  ...
  const applyInserting = () => {
    if (dragItemState.mode != 'NONE' && dragItemState.toIndex != -1) {
      const tempLiveList = layoutSetting.liveList.filter(
        (loginName) => !(dragItemState.mode === 'MOVE' && loginName == dragItemState.cursor),
      );
      onChange({
        target: {
          name: 'liveList',
  ...
  return (
    <div className={cx('SceneLayoutSettingPanel')}>
      <header>
        <div className={styles.left}>레이아웃 설정</div>
        <div className="right">
          <TransparentButton onClick={onClose}>취소하고 닫기</TransparentButton>
          <BasicButton onClick={onSubmitHandler} disabled={layoutSetting.liveList.length < 1}>
            저장
          </BasicButton>
        </div>
      </header>
      <div className={styles.content}>
        <div
          className={cx('layoutPreview', layoutSetting.viewLayout, {
            isDragging: dragItemState.mode != 'NONE',
            hideLast:
              dragItemState.mode === 'ADD' &&
              dragItemState.toIndex !== layoutSetting.liveList.length,
          })}
          style={{ '--gridColumns': gridColumns, '--gridRows': gridRows }}
          onMouseLeave={(e) =>
            dragItemState.mode == 'ADD' && setDragItemState({ ...dragItemState, toIndex: -1 })
          }
        >
        ...
        <div className={styles.aside}>
          <SceneLayoutContentSection className={styles.layoutSelector} title="화면 구성">
            <SceneLayoutSelect
              name="viewLayout"
              value={layoutSetting.viewLayout}
              onChange={onChange}
            />
          </SceneLayoutContentSection>
          <SceneLayoutContentSection
            className={styles.waktaverseMemberList}
            title="같이 볼 멤버 선택"
          >
            <ul className={styles.memberList}>{isLoading ? <Spinner /> : memberList}</ul>
          </SceneLayoutContentSection>
        </div>
      </div>
      {dragItemState.mode == 'ADD' && (
        <CursorMemberItem
          ...
          nickname={
            allMemberList.find((member) => member.twitchLoginName === dragItemState.cursor).nickname
          }
          profileImg={
            allMemberList.find((member) => member.twitchLoginName === dragItemState.cursor)
              .twitchProfileImage
          }
        />
      )}
...
function SceneLayoutContentSection({ className, title, children }) {
  return (
    <div className={cx('SceneLayoutContentSection', className)}>
...
function SceneLayoutSelect({ name, value, onChange = () => {} }) {
  ...
  const optionList = options.map((item) => (
    <div
      key={item.value}
      className={cx('option', { selected: item.value === value })}
      onClick={(e) =>
        onChange({
          target: {
            name,
  ...
  return <div className={cx('SceneLayoutSelect')}>{optionList}</div>;
}
...
function MemberItem({
  ...
  return (
    <div
      className={cx('MemberItem', className, {
        liveYoutube: broadcaster === 'YOUTUBE',
        liveTwitch: broadcaster === 'TWITCH',
        liveChzzk: broadcaster === 'CHZZK',
        liveAfreeca: broadcaster === 'AFREECA',
        selected,
      })}
      draggable="false"
