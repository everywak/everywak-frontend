...
class TwitchChatClientCore {
  ...
  constructor({
    ...
    this.hostname =
      this.ircServer === 'wss://irc-ws.chat.twitch.tv'
        ? 'tmi.twitch.tv'
        : this.ircServer.replace('wss://', '').replace(/:\d+$/, '');
  ...
  onClose = (e) => {
    ...
    if (this.oauth != undefined && this.oauth != '' && !e.wasClean) {
      // retry login now,2,4,8 secs..
  ...
  onMessage = (e) => {
    ...
    if (process.env.NODE_ENV == 'development') {
      console.log('Twitch IRC WebSocket has sended a message.');
      console.log(res);
    }
  ...
  handleChatMessage = (data) => {
    ...
    if (data[2] == 'PRIVMSG') {
      ...
      const { loginName } = data[0].match(
        /:(?<loginName>[\w\d-_]+)!\k<loginName>@\k<loginName>\.tmi\.twitch\.tv/,
      )?.groups || { loginName: '' };
      if (this.myUserInfo.loginName === loginName) {
        // is me
        this.setIRCState(TwitchChatClientCore.AUTHORIZED);
      }
    }
  ...
  sendMessage = (data) => {
    if (
      [
        TwitchChatClientCore.CONNECTED,
        TwitchChatClientCore.AUTHORIZED,
        TwitchChatClientCore.JOINED,
      ].includes(this.IRCState)
    ) {
  ...
  receiveChat = async ({ tag, id, msg, mine = false, channelId = '' }) => {
    ...
    const regexMsg = new RegExp(
      `:(?<loginName>[\\w\\d-_]+)!\\k<loginName>@\\k<loginName>\\.${this.hostname.replace(/\./g, '\\.')}`,
    );
    ...
    const displayName = channel
      ? `${channel.name[0]}: ${tags['display-name']}`
      : tags['display-name'];
    ...
    if (!this.myUserInfo.blockedUsers.find((user) => user.user_login === chat.userId)) {
  ...
  updateEmoteSet = async (emoteSetsRaw) => {
    ...
    if (emoteSets.length > 0 && this.emoteSets.length === 0) {
      ...
      console.log(
        'emotesets',
        this.emoteSets.map((em) => ({ name: em.name, type: em.emote_type })),
      );
  ...
  parseEmote = (data) => {
    ...
    // parse emote data
    data
      .replace(';', '') // id:pos/id:pos,pos/id:pos,pos
      .split('/')
      .map((em) => {
        // [ id:pos, id:pos,pos, id:pos,pos ]
