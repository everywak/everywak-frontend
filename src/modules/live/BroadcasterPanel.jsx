    ...
    channelId && (
      <div className="BroadcasterPanel">
        <BasicSelect
          className="tabList"
          options={[
            { name: '다시보기 및 클립', value: 'videoContent' },
            { name: '최근 예측', value: 'prediction' },
            { name: '바로가기', value: 'panel' },
          ]}
          name="tab"
          value={currentTab}
...
function PanelTab({ channelId }) {
  ...
  return <div className="PanelTab tab">{panel}</div>;
}
  ...
  return (
    <div className="VideoContentTab tab">
      <ContentPanel className="main replay" title="다시보기" width="spaceBetween">
        <VideoContentList
          options={{ type: 'youtubeVOD', twitchId: channelId }}
      ...
      <ContentPanel className="main clip" title="최근 클립" width="spaceBetween">
        <VideoContentList
          options={{ type: 'youtubeClip', twitchId: channelId }}
          backgroundColor="#fafafa"
          hideProfileCircle
        />
      </ContentPanel>
...
function FooterCopyright() {
  return (
    <p className="FooterCopyright">
      에브리왁굳 우왁굳 생방송 페이지는 YouTube, Twitch 및 AfreecaTV의 서드파티 사이트로 YouTube,
      Twitch 및 AfreecaTV에서 운영하는 사이트가 아닙니다.
      <br />
      'YouTube' 및 '유튜브'는 YouTube, LLC의 등록상표이며 'Twitch' 및 '트위치'는 Twitch Interactive,
      Inc.의 등록상표이고 'AfreecaTV' 및 '아프리카TV'는 SOOP의 등록상표입니다.
      <br />
      &nbsp;
      <br />
      에브리왁굳 © 2020-2024.{' '}
