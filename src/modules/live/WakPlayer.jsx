...
class WakPlayer extends Component {
  ...
  onTwitchOffline = () => {
    // TODO: 오프라인시 임베드 제거하고 오프라인 이미지 표시하는 등 최적화 처리할 것
    this.playerContext.notify({
      type: 'CHANGE_ONLINESTATE',
      value: 'OFFLINE',
    });
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (
      ...
      this.embed = (
        <TwitchPlayer
          id={`streamembed_${this.props.channelId}`}
          className="stream"
          ...
          hideControls={true}
          muted={true}
          onReady={this.onTwitchReady}
          onPlay={this.onTwitchPlay}
          onPlaying={this.onTwitchPlaying}
          onPause={this.onTwitchPause}
          onOnline={this.onTwitchOnline}
          onOffline={this.onTwitchOffline}
        />
      );
    }
    return (
  ...
  render() {
    const { onChangeOverlayState, overlayStyle, onClickOverlay } = this.props;
    const { broadcaster, player, onlineState } = this.state;
...
class WakPlayerOverlay extends Component {
  ...
  loopOverlayLife = () => {
    ...
    this.overlayLife =
      this.state.settingPanel === 'CLOSED' ? Math.max(this.overlayLife - 50, 0) : 100;
  };
   ...
   */
  play = () => {
    const { broadcaster, player } = this.props;
   ...
   */
  onTouch = (e) => {
    if (!(e.target == e.currentTarget || e.target.className === 'bottom')) {
      return;
    }
    const touchedState = this.state.opened;
    setTimeout(() => {
      if (touchedState !== this.state.opened) {
        return;
      }
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    // 오버레이 상태 변경 감지
    if (prevState.opened !== this.state.opened || prevState.expanded !== this.state.expanded) {
  ...
  render() {
    const { overlayStyle, onClickOverlay } = this.props;
    const {
      volPressed,
      settingPanel,
      quality,
    ...
    const videoQualityList = qualities.map((q) => (
      <li
        className="item vq"
        data-vq={q.name.indexOf('(source)') === -1 ? q.name : 'chunked'}
        onClick={(e) => this.setQuality(e.currentTarget.dataset.vq)}
      >
        {qualityList[q.name] || q.name}
      </li>
    ));

    return (
      <div
        className={cx('WakPlayerOverlay', 'overlayArea', {
          opened: opened || expanded || volPressed || settingPanel !== 'CLOSED',
          styleVolumeOnly: overlayStyle === 'volumeOnly',
        })}
        ...
        <div className="controller" onTouchEnd={this.onTouch} onClick={onClickOverlay}>
          <div className="bottom">
            <div className="bottomLeft">
              <span
                className={cx('ctlBtn', 'playpause', {
                  paused: !(playerState === 'PLAYING' || playerState === 'PLAY'),
                })}
                onClick={(e) => {
                  playerState === 'PLAYING' ? this.pause() : this.play();
                }}
              >
                <svg viewBox="0 0 24 24" fill="white" width="24px" height="24px" className="play">
                  <path d="M0 0h24v24H0z" fill="none" />
                  <path d="M8 5v14l11-7z" />
                </svg>
                <svg viewBox="0 0 24 24" fill="white" width="24px" height="24px" className="pause">
                  <path d="M0 0h24v24H0z" fill="none" />
              ...
              <WakPlayerVolumeBar
                ...
                onChangeValue={this.onChangeVolumeHandler}
                onChangePressed={(val) => {}}
              />
            </div>
            <div className="bottomRight">
              <span
                className="ctlBtn setting"
                onClick={(e) => {
                  this.setSettingPanel(settingPanel === 'CLOSED' ? 'MAIN' : 'CLOSED');
                }}
              >
                <svg
              ...
              <div className={cx('popup', 'settingPopup', { opened: settingPanel !== 'CLOSED' })}>
                <ul className="itemList">
                  <li
                    className="item videoQuality subList"
                    onClick={(e) => this.setSettingPanel('VIDEO_QUALITY')}
                  >
                    화질
              ...
              <div
                className={cx('popup', 'videoQualityPopup', {
                  opened: settingPanel === 'VIDEO_QUALITY',
                })}
              >
                <ul className="itemList">{videoQualityList}</ul>
              </div>
            </div>
          </div>
        </div>
