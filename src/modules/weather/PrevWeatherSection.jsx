...
function PrevWeatherSection({ length = 4 }) {
  const [weatherInfo, setWeatherInfo] = useState(/** @type {{date: string, data: []}[]} */
  ...
  useEffect(() => { Array(length).fill(0).forEach((_, i) => updateIsedolWeather(i + 1)); }, []);
  console.log(weatherInfo)

  const weatherList = weatherInfo.map((item, i) => item ?
      [<PrevWeatherList key={`${i}_false`} isLoading={false} items={item.data} date={item.date} />, <div className="weatherBorder" />] :
      [<PrevWeatherList key={`${i}_true`} isLoading={true} items={null} />, <div className="weatherBorder" />]
    )

  return (
    <section className="PrevWeatherSection">
      {weatherList.flat()}
    </section>
  );
}
