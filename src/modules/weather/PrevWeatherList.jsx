...
function PrevWeatherList({ date = '000000', items, isLoading }) {
  ...
  const list = items && items.map(item => <PrevWeatherItem key={item.nickname} name={item.nickname} state={item.rawInfo} weather={item.weather} />);

  const dateTarget = new Date('20' + date.slice(0, 2), parseInt(date.slice(2, 4)) - 1, date.slice(4, 6));
  ...
  return (
    <ul className="PrevWeatherList">
      {isLoading ? 
        <>
        <SkeletonLoader skeleton={<div className="date skeleton"><div className="skeletonItem">00/00(ㅇ)</div></div>} length={1} />
