...
async function updateWaktoonEpisodeChart({ orderBy = 'up', duration = 'daily' }) {
  try {
    ...
    const resToday = await service.getWaktoonEpisodeChart({
      orderBy,
      perPage: 100,
      after: now,
      before: now - targetDuration,
    });
    const resYesterday = await service.getWaktoonEpisodeChart({
      orderBy,
      perPage: 100,
      after: now - targetDuration,
      before: now - 2 * targetDuration,
    });

    if (resToday.status !== 200) {
      throw resToday;
    }
    if (resYesterday.status !== 200) {
      throw resYesterday;
    }
...
const skeletonLarge = (
  <li className="WaktoonChartLargeItem skeleton">
    <a>
      ...
      <div className="infoArea">
        <div className="descArea">
          <div className={cx('rankAmount', 'new')}>
            <span className="skeletonItem">aaa</span>
          </div>
          <div className="titleWrapper">
            <div className="title">
              <span className="skeletonItem">asdfasdfasdfasdfasdf</span>
              <div className="author skeletonItem">asdfasdf</div>
            </div>
...
function WaktoonChartList({
  ...
  useEffect(() => {
    setfilteredItemList(
      itemList.filter(
        (item) => !keyword || item.title.includes(keyword) || item.author.includes(keyword),
      ),
    );
  }, [itemList, keyword]);
