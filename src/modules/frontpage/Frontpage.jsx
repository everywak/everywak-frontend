...
function DesktopFrontpage() {
  ...
  const leftLiveList = isedol.slice(0, 3).map(id => <LivePreview key={`livepanel_${id}`} channelId={id} />);
  const rightLiveList = isedol.slice(3, 6).map(id => <LivePreview key={`livepanel_${id}`} channelId={id} />);
    ...
    <main className="content">
      <section className="twoColumns">
        <section className="content">
          <ContentPanel className="main waktaverseYoutube" title="왁타버스 유튜브" /*moreLink="/video"*/ moreLabel="왁굳TV에서 더 보기" width="spaceBetween">
            <VideoContentList className="waktaverseVideoList" size="small" options={{ type: 'youtubeVideo' }} />
          </ContentPanel>
          <ContentPanel className="main wakMusic" title="에브리뮤직" moreLink="https://everywak.kr/music/concert" moreLabel="이세돌 콘서트 바로가기" width="spaceBetween">
        ...
        <aside>
          ...
          <AsidePanel title="게시판">
            ...
            ].map(app => <AppListItem size="small" {...app} />)}
          </AsidePanel>
        </aside>
      </section>
...
function MobileFrontpage() {
  ...
  return (<div className="FrontPage mobile">
    <header>
