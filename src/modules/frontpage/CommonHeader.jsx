...
function CommonHeader({
  className, title, isBeta = false, categories, subtitle, description, moreLink, children, 
}) {

  const categoryList = categories.map(({key, ...item}) => <li className="categoryItem" key={key}>{
      item.href.match(/^\//) ? 
    ...
    <header className={cx('CommonHeader', className)}>
      <div className="headerWrapper">
        <div className="titleWrapper">
          <h1 className="title">{title}{isBeta && <span className="beta">βeta</span>}</h1>
