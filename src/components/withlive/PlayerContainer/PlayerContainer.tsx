...
export const PlayerContainer = () => {
  const { channels, watchingChannels, isEnabledMultiView, multiViewLayout } =
    useWithliveValues();

  const _watchingPlayers = watchingChannels
    .map((watchingChannel) => {
      const channel = channels.find(
        (channel) => channel.memberId === watchingChannel.memberId,
      );
      return {
  ...
  const firstPlayerSize =
    isEnabledMultiView &&
    !['one-side-r', 'one-side-l'].includes(multiViewLayout)
      ? 'normal'
      : 'full';
  ...
  return (
    <section
      className={clsx(styles.container, {
        ...
        [styles.oneSide]:
          isEnabledMultiView && multiViewLayout.startsWith('one-side'),
        [styles.oneSideR]:
          isEnabledMultiView && multiViewLayout === 'one-side-r',
        [styles.oneSideL]:
          isEnabledMultiView && multiViewLayout === 'one-side-l',
        [styles.oneSideT]:
          isEnabledMultiView && multiViewLayout === 'one-side-t',
        [styles.oneSideB]:
          isEnabledMultiView && multiViewLayout === 'one-side-b',
