  ...
  useEffect(() => {
    const _embed = (
      <TwitchPlayer
        ...
        className={cx('player', 'twitchLive', className)}
        channel={channelId}
        parent={[domain]}
        width="100%"
        height="100%"
  ...
  useEffect(() => {
    if (player) {
      ...
      const videoContentInterface = {
        ...
        getQuality: () => ({
          label: qualityIdTwitch[player.getQuality()],
          value: player.getQuality(),
        }),
        getQualities: () =>
          player
            .getQualities()
            .map((item) => ({
              label: qualityIdTwitch[item.name],
              value: item.name.includes('(source)') ? 'chunked' : item.name,
            })),
        getPlaybackInfo: () => {
