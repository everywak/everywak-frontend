...
function YoutubeVideoPlayer ({
  ...
  useEffect(() => {
    const _embed = <YouTube id={`streamembed_${videoId}`} className={cx('player', 'youtubeVideo', className)} videoId={videoId} 
      style={{}} 
      title={'title'} 
      loading={undefined} 
      opts={{
      ...
      onReady={e => {e.target.mute();setPlayer(e)}}
      onPlay={_onPlaying} 
      onPause={_onPause}  /*
      onEnd={func} 
      onError={func} 
  ...
  useEffect(() => {
    if (player) {
      ...
      handlers.onReady({
        ...
        getQuality: () => ({label: qualityIdYoutube[_player.getPlaybackQuality()], value: _player.getPlaybackQuality()}),
        getQualities: () => _player.getAvailableQualityLevels().map(item => ({label: qualityIdYoutube[item], value: item})),
      });
    }
  }, [player]);
