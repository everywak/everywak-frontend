...
const formatHMMSSString = (seconds) =>
  (seconds >= 3600 ? `${parseInt(seconds / 3600)}:` : '') +
  `${parseInt(seconds / 60) % 60}:${('0' + parseInt(seconds % 60)).slice(-2)}`;
      ...
      mediaType === 'youtubeLive' &&
        ...
        onChangeState({
          target: {
            name: 'startedTimeStamp',
            value: Math.min(
              Date.now() - player.getCurrentTime() * 1000,
              playerState.startedTimeStamp,
            ),
          },
        });
      mediaType === 'youtubeLive' &&
  ...
  const controlButtonItems = controlButtons.map((item) => {
    if (item.type === 'toggle') {
      ...
      return (
        ...
        >
          {item.value ? <EnabledIcon className="iconImg" /> : <DisabledIcon className="iconImg" />}
        </ToggleButton>
      );
    } else {
    ...
    <div className={cx('VideoContentPlayer', className)} ref={_el} {...rest}>
      ...
      <div
        className={cx('overlay', {
          iframemode: mediaType === 'chzzkLive' || mediaType === 'afreecaLive',
          hover: playerOptions.hovering || volumeBarClicked || playerOptions.openedSettings,
          simple: playerSize === 'simple',
        })}
        onClick={onClickOverlay}
      >
        {mediaType === 'twitchLive' && player && (
          <VideoContentPlayerPlaybackInfoPanel player={player} />
        )}
        <div className="mediaInfo">
          ...
          <div className="headerButtons">{headerButtonItems}</div>
        </div>
        ...
        <div className={cx('controls', { hideProgress: mediaType === 'twitchLive' })}>
          ...
          <div className="buttonArea">
            <div className="left">
              <BasicButton
                className="playpause"
                description={playerState.playing ? '일시정지' : '재생'}
                background="transparent"
              ...
              <ToggleButton
                ...
                description={playerState.isMuted ? '음소거 해제' : '음소거'}
                background="transparent"
              >
                {playerState.isMuted ? <VolumeOffRoundedIcon className="iconImg" /> : volumeIcon}
              </ToggleButton>
              <div className={cx('volumeWrapper', { show: volumeBarClicked })}>
                <ProgressBar
                  ...
                  onChange={onChangeState}
                  onMouseDown={(e) => {
                    playerState.isMuted && toggleMute();
                    setVolumeBarClicked(e);
                  }}
                  actionType="drag"
                />
              </div>
            ...
            <div className="right">
              <ToggleButton
                className="caption"
                name="showCaption"
                value={playerOptions.showCaption}
                onChange={onChangeOption}
                description={playerOptions.showCaption ? '자막 끄기' : '자막 켜기'}
                background="transparent"
              >
              ...
              <ToggleButton
                className={cx('settings', { on: playerOptions.openedSettings })}
                name="openedSettings"
                value={playerOptions.openedSettings}
                onChange={onChangeOption}
                description="옵션"
                background="transparent"
              >
...
function VideoContentPlayerPlaybackInfoPanel({ player }) {
  ...
  return (
    <div className="VideoContentPlayerPlaybackInfoPanel">
